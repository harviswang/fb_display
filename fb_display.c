...
void* convertRGB2FB(int fh, unsigned char *rgbbuff, unsigned long count,
		int bpp, int *cpp);
void blit2FB(int fh, void *fbbuff, unsigned int pic_xs, unsigned int pic_ys,
		unsigned int scr_xs, unsigned int scr_ys, unsigned int xp,
		unsigned int yp, unsigned int xoffs, unsigned int yoffs, int cpp);
static void dumpinfo(struct fb_fix_screeninfo *fi,
		struct fb_var_screeninfo *vi);

/*
 * 
 * rgbbuff: 24bits RGB颜色缓冲区
 * x_size: 屏幕在x方向上的大小
 * y_size: 屏幕在x方向上的大小
 * x_pan: x方向开始绘制的地方
 * y_pan: y方向始绘制的地方
 ...
 * y_offs: 
 */
void fb_display(unsigned char *rgbbuff, unsigned int x_size,
		int unsigned y_size, unsigned int x_pan, unsigned int y_pan, int x_offs,
		int y_offs) {
	...
	/* read fb_fix_screeninfo */
	getFixScreenInfo(fh, &fi);

	/* read current video mode */
	getVarScreenInfo(fh, &var);

	/* dump fb_var_screeninfo info */
	dumpinfo(&fi, &var);
	...
	/* blit buffer 2 fb */
	fbbuff = convertRGB2FB(fh, rgbbuff, x_size * y_size, var.bits_per_pixel,
			&bp);
	blit2FB(fh, fbbuff, x_size, y_size, var.xres, var.yres, x_pan, y_pan,
			x_offs, y_offs, bp);
	
	/*
	 * just use the first framebuffer, not using ping-pong buffer
	 */
	var.yres_virtual = var.yres;
	var.yoffset= 0;
	if (ioctl(fh, FBIOPAN_DISPLAY, &var) < 0) {
		printf("active fb pan_display failed\n");
	}
	free(fbbuff);
...
inline unsigned short make16color(unsigned char r, unsigned char g,
		unsigned char b) {
		...
		int bpp, int *cpp)
{
	...
	case 8:
		*cpp = 1;
		...
		for (i = 0; i < count; i++)
			c_fbbuff[i] = make8color(rgbbuff[i * 3], rgbbuff[i * 3 + 1],
					rgbbuff[i * 3 + 2]);
	...
	case 15:
		*cpp = 2;
		...
		for (i = 0; i < count; i++)
			s_fbbuff[i] = make15color(rgbbuff[i * 3], rgbbuff[i * 3 + 1],
					rgbbuff[i * 3 + 2]);
	...
	case 16:
		*cpp = 2;
		s_fbbuff = (unsigned short *) malloc(count * sizeof(unsigned short));
		for (i = 0; i < count; i++)
			s_fbbuff[i] = make16color(rgbbuff[i * 3], rgbbuff[i * 3 + 1],
					rgbbuff[i * 3 + 2]);
		fbbuff = (void *) s_fbbuff;
		break;
	case 24:
